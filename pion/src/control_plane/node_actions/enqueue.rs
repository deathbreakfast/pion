...
async fn ensure_node_enqueueable(
    ...
    let node = PionControlPlaneNode::get(node_id, valence, valence::use_!("In **Pion control plane**, we **load Pion Control Plane Node** so the application can decide what to do next in this workflow. The result is used by **Pion control plane** logic—not necessarily displayed on a page unless that feature’s UI shows it."))
        .await
...
pub async fn enqueue_node_action_idempotent(
    ...
    PionNodeActionCommand::upsert(&cmd_id, row, valence, valence::use_!("When **Pion control plane** needs to persist work, we **save Pion Node Action Command** so the next step in that feature can continue with the latest values. People and services allowed for **Pion control plane** use this data for that workflow—not as a general export of unrelated personal fields."))
        .await
