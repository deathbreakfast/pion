...
async fn seed_pool_columns(
    ...
    PionControlPlaneVirtualPool::upsert(POOL, pool, valence, valence::use_!("When **Pion control plane** needs to persist work, we **save Pion Control Plane Virtual Pool** so the next step in that feature can continue with the latest values. People and services allowed for **Pion control plane** use this data for that workflow—not as a general export of unrelated personal fields.")).await?;
    let map =
        PionControlPlanePoolCellMap::new(POOL.to_string(), CELL.to_string(), 1, true, now, now)?;
    PionControlPlanePoolCellMap::upsert(&format!("{POOL}:{CELL}"), map, valence, valence::use_!("When **Pion control plane** needs to persist work, we **save Pion Control Plane Pool Cell Map** so the next step in that feature can continue with the latest values. People and services allowed for **Pion control plane** use this data for that workflow—not as a general export of unrelated personal fields.")).await?;
    Ok(())
}
...
async fn seed_node_with_status(
    ...
    PionControlPlaneNode::upsert(spec.node_id, node, valence, valence::use_!("When **Pion control plane** needs to persist work, we **save Pion Control Plane Node** so the next step in that feature can continue with the latest values. People and services allowed for **Pion control plane** use this data for that workflow—not as a general export of unrelated personal fields.")).await?;
    Ok(())
}
...
async fn missing_pool_and_unmapped_cells_are_distinguished() -> anyhow::Result<()> {
    ...
    PionControlPlaneVirtualPool::upsert(POOL, pool, &v, valence::use_!("When **Pion control plane** needs to persist work, we **save Pion Control Plane Virtual Pool** so the next step in that feature can continue with the latest values. People and services allowed for **Pion control plane** use this data for that workflow—not as a general export of unrelated personal fields.")).await?;
    let err = resolve_eligible_pool_nodes(POOL, &NodeHardwareRequirements::default(), &v)
        .await
