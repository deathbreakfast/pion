...
pub async fn resolve_eligible_pool_nodes(
    ...
    let pool = PionControlPlaneVirtualPool::get(pool_id, valence, valence::use_!("In **Pion control plane**, we **load Pion Control Plane Virtual Pool** so the application can decide what to do next in this workflow. The result is used by **Pion control plane** logic—not necessarily displayed on a page unless that feature’s UI shows it."))
        .await
...
async fn enabled_cell_ids(
    ...
    let cells = PionControlPlanePoolCellMap::query(valence, valence::use_!("In **Pion control plane**, we **list Pion Control Plane Pool Cell Map** so the product can show or process the matching set for this workflow. Callers allowed for **Pion control plane** use the list; it is not a public dump of every field to anonymous visitors."))
        .await
...
async fn reachable_nodes_in_cells(
    ...
    let in_cells = PionControlPlaneNode::query(valence, valence::use_!("In **Pion control plane**, we **list Pion Control Plane Node** so the product can show or process the matching set for this workflow. Callers allowed for **Pion control plane** use the list; it is not a public dump of every field to anonymous visitors."))
        .await
